...
main(int argc, char *argv[]) 
{
	...
	if(strcmp(entry[0].value, "mkdir") == 0){ // mkdir
		for(i = 1;  i <= m; i++) {
			if(strcmp(entry[i].value, "\0") != 0){ // checks to make 
				strcpy(pathName, cwd);
				strcat(pathName, "/");
				strcat(pathName, entry[i].value);
				printf("command is %s, Dir name is \"%s\"\n", entry[0].value, pathName);
				r = mkdir(pathName, 0777); // was going to use functions from sys/stat.h (S_IRWXU, S_IRWXG, S_IROTH, S_IXOTH) but friend suggested 0x16D instead
				if(r != 0){
					i = m;
				}
			}
		...
		r = 0;  // this make it so system knows that command succeeded
	...
	if(r) {
		...
		if(failed) printf("<h2>Command failed!</h2>");
		else printf("<h2>Command succeeded!</h2>");
	}
	failed = 0;
